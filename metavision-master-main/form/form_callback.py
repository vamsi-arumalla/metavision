...
def register_form_callback(app, cache):
    ...
    def handle_navigation_and_processing(n_landing, n_vis, n_exp, n_back, processed):
        ...
        # Handle processing completion
            ...
            return "postprocessing", dash.no_update
    ...
    def handle_processing(
        ...
        if btn_id == "run-button":
            ...
            try:
                ...
                try:
                    if chunked_file:
                        # For chunked files, read directly from disk
                        df = pd.read_csv(os.path.join("workspaces", chunked_file), on_bad_lines='skip')
                        logger.info("Used direct CSV reading for chunked file")
                    else:
                        # For standard uploads, decode the content
